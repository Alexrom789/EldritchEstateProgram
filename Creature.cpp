...
Creature::Creature(int av, int an, int hp, string nm)
{
	attackDVal = av;
	attackDNum = an;
	healthPoints = hp;
...
int Creature::rollToAtk()
{
	vector<int> storedRolls(4);

	for (int i = 1; i <= attackDNum; i++)
	{
		storedRolls[i] = rand() % attackDVal + 1;
	}
	int dieTotal = storedRolls[1] + storedRolls[2] + storedRolls[3];
	//cout << "The " << name << "'s attack result is " << dieTotal << endl;
	cout << endl;
	return dieTotal;
}
...
void Creature::setDVal(int newDval)
{
	attackDVal = newDval;
}
